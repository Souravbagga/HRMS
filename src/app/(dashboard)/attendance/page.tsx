    ...
    records.length > 0
      ? (
          records.reduce((sum, r) => sum + (r.total_hours ?? 0), 0) /
            records.filter((r) => r.total_hours).length || 0
        ).toFixed(1)
      : "0.0";
  ...
  return (
    <div className="flex flex-col gap-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight text-indigo-500">
            Attendance
          </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <div className="lg:col-span-8 grid grid-cols-1 sm:grid-cols-3 gap-6">
          {stats.map((s) => (
            <Card
              key={s.title}
              className="rounded-2xl border-none shadow-sm bg-card hover:-translate-y-0.5 transition-transform"
            >
              <CardContent className="p-6 flex flex-col items-center text-center h-full justify-center">
                ...
                <p className="text-[11px] text-muted-foreground/60 mt-2 font-semibold italic">
                  {s.sub}
                </p>
              </CardContent>
            </Card>
      ...
      <div className="flex bg-card flex-col border border-border/60 rounded-2xl shadow-sm overflow-hidden">
        <div className="p-5 border-b border-border/40 bg-muted/20 flex justify-between items-center">
          ...
          <div className="flex gap-2">
            <Badge
              variant="outline"
              className="bg-emerald-500/5 text-emerald-600 border-emerald-500/20 px-3"
            >
              Present: {presentCount}
            </Badge>
            <Badge
              variant="outline"
              className="bg-rose-500/5 text-rose-600 border-rose-500/20 px-3"
            >
              Late: {lateCount}
            </Badge>
          </div>
